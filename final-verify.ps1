$queries = @(
    @{name="Servicios UP"; q="count(up{job=`"bodegahub-servicios`"} == 1)"},
    @{name="Servicios DOWN"; q="count(up{job=`"bodegahub-servicios`"} == 0)"},
    ...
    @{name="Event Loop Lag p99"; q="nodejs_eventloop_lag_p99_seconds{job=`"bodegahub-servicios`"} * 1000"},
    @{name="Heap Used MB"; q="nodejs_heap_size_used_bytes{job=`"bodegahub-servicios`"} / 1048576"},
    ...
    @{name="PG Conexiones"; q="pg_stat_database_numbackends{datname=`"bodegahub`"}"},
