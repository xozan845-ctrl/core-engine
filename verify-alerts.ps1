$checks = @(
    @{name="Servicios UP"; query="count(up{job=`"core-engine-servicios`"} == 1)"},
    @{name="Servicios caidos"; query="count(up{job=`"core-engine-servicios`"} == 0)"},
    ...
    @{name="Event Loop Lag p99"; query="nodejs_eventloop_lag_p99_seconds{job=~`"core-engine-servicios`"} * 1000"},
    ...
    @{name="Heap Used MB"; query="nodejs_heap_size_used_bytes{job=~`"core-engine-servicios`"} / 1048576"},
    @{name="Conexiones PG"; query="pg_stat_database_numbackends{datname=`"core-engine`"}"},
