...
async function bootstrap(): Promise<void> {
  ...
  app.useGlobalInterceptors(
    new TrazabilidadInterceptor(app.get(MetricsService), NOMBRE_SERVICIOS.CATALOG),
  );
  app.useGlobalFilters(new DomainErrorFilter());
  const puerto = Number(process.env.PORT ?? PUERTOS.CATALOG);
  ...
  Logger.create(NOMBRE_SERVICIOS.CATALOG).info({ msg: 'catalog-service listo', puerto });
}
